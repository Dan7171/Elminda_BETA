...
"rs" : 42  ,  # random state
    ...
    "X_version": 1,
    # 1 = no dimension reduction (1000 features) / 2 = medium dimension reduction (500 features), 3 = significant reduction (100-200 features)

    "split_rows": 'normal',
    # 'normal' = normal train test split (dont drop anything) / 'h1'- drop rows where Treatment != 'h1' and then use train test split.
    ...
    "scoring_method": 'roc_auc',  # "accuracy" /'f1'/ 'roc_auc'

    "both": False,
    # (for now works on classificatin only) True- train on both research 1 and research 2 data , False- train on research 2 only.IMPORTANT: if set to True, use X_version = 1 only!!!
    ...
    "n_iter": 15,  # param for randomized cv - num of combinations to try in randomized search
