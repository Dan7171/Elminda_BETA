...
def CV_Score(y_true, y_pred):
    ...
    if my_scorer == 'roc_auc':
        ...
        cvscore = fbeta_score(y_true, y_pred,
                              args['beta'])  # beta = weight given to recall (weight given to precision is always 1)
...
#        'classifier__n_estimators': [5000],  # number of trees
    ...
    # RANDOM FOREST + kbest
    param5b = {
        "kBest__k": [20, 50, 100, 300],
        'classifier__n_estimators': [100, 500, 2000],  # number of trees
        "classifier__n_jobs": [-1],
